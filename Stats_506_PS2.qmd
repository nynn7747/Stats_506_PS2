...
play_dice2 <- function(x) {
    ...
    get_scores <- ifelse(results == 3 | results == 5, results * 2, 0)
...
play_dice3 <- function(x) {
    ...
    winnings <- as.numeric((-2)*x + 6*number_3 + 10*number_5) 
    return(winnings)
}
...
**I think this is not a fair game based on the expected value and Monte Carlo simulations. This game is in favor of the player based on the positive values in the expected value and the simulation result.**

**Expected value:**

$E(x) = x \times \left[(-2) \times \frac{4}{6} + (2 \times 3 - 2) \times \frac{1}{6} + (2 \times 5 - 2) \times \frac{1}{6}\right]$

**This simplifies to:**

$E(x) = \frac{2x}{3} > 0$

**With Monte Carlo simulation, based on 1000 times simulations, no matter we have 10, 100, or 1000 rolls, the winnings are larger than 0.**
...
**The estimated coefficient for torque is -0.0507 with a significant *p*-value. This means that for every one-unit increase in torque, the highway miles per gallon decrease by 0.0507 units, after controlling for horsepower, dimensions (height, length, width), and the year of release. Therefore, cars with higher torque generally have lower fuel efficiency on highways.**

```{r}
#| code-fold: true
library(tidyverse)
cars_gas <- cars_gas %>%
  mutate(Years_release = 2024 - Year)
cars_gas$`Years_release` <- as.factor(cars_gas$`Years_release`)
form1 <- Highway_mpg ~ Torque + Horsepower + Height + Length + Width + `Years_release`
...
**The interaction coefficient for Torque:Horsepower is 0.0001124. This means that the association between torque and highway MPG is related to horsepower. For every additional unit of horsepower, the negative impact of torque on highway MPG decreases by 0.0001124 units.**

```{r}
#| code-fold: true
form2 <- Highway_mpg ~ Torque + Horsepower + Height + Length + Width + `Years_release` + Torque:Horsepower
...
# Generate an interaction plot at Year= 13
emmip(model2, Horsepower ~ Torque, at = list(Torque = torque_vals, Horsepower = hp_vals, `Year_release` = 13))
